...
| UVC — USB camera | 🟡 Implemented end to end: descriptors parsed during enumeration, `getVideoStreams()` / `espUsbHostSelectVideoStream()` to choose a format, `videoStart()` to negotiate Probe/Commit and stream, `onVideoFrame()` to receive whole frames. Two limits in arduino-esp32 3.3.11's precompiled host stack keep this away from real webcams, and neither can be worked around from a sketch: a 256-byte configuration-descriptor ceiling, and — on a full-speed port — isochronous IN packets limited to about 120 bytes. Verified against a small `EspUsbDevice` camera peer |
...
- **UVC / USB cameras:** implemented and verified end to end, but not against a real webcam, because two limits in Arduino-ESP32's precompiled USB Host stack stand in the way and neither can be changed from a sketch or from an EspUsbHost build option — both belong to the build configuration used to generate the precompiled libraries. First, `CONFIG_USB_HOST_CONTROL_TRANSFER_MAX_SIZE=256`: a device whose configuration descriptor exceeds 256 bytes fails during enumeration, before a class driver can start, and the Logitech C920 tested here has a 1,974-byte descriptor. Second, on a full-speed port, `CONFIG_USB_HOST_HW_BUFFER_BIAS_PERIODIC_OUT` leaves about 120 bytes for IN, so a camera's isochronous IN endpoint either fails to claim or claims and delivers nothing; a high-speed port is not affected by that one. Larger limits are expected in a future Arduino-ESP32 release, but which release that is, and whether the values that ship are enough for a real webcam, can only be established once it is out. See [docs/usb-host-advanced.md](docs/usb-host-advanced.md) for the measurements.
...
Measured on an ESP32-S3 (full-speed OTG) with `tests/manual/vendor_bulk_throughput`: the queue reaches 1.151 MB/s, about 95% of the 1.216 MB/s full-speed bulk ceiling, and a depth of 2 is enough to stay there at any transfer size. Synchronous `vendorWrite()` reaches the same figure only with large transfers and drops to 0.92 MB/s at 512 bytes, where per-transfer latency dominates. Depths beyond 2 did not help on full speed. (All figures are decimal MB/s, 10^6 bytes per second.)
...
Measured on an ESP32-P4 high-speed port reading an ESP32-P4 device: the one-packet continuous read gives 6.10 MB/s, a single 32 KB transfer 14.77 MB/s, and a depth of 4 with 32 KB transfers 24.45 MB/s — the point where that device saturates. Both dimensions matter on the IN side, unlike the OUT side where a depth of 2 is enough on its own.
