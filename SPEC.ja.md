...
テストは、実機が必要なUSB Host動作と、実機なしで確認できるロジックを分けて考えます。

ローカル環境では、UnityやArduTestなど特定のテストフレームワークに依存しない、シンプルなテキストベースのテストを基本にします。
テスト対象は結果を標準出力またはSerialへ出し、pytest側が最後の `OK` / `NG` や必要なログを確認します。
...
この方式により、`lang-ship:host:host`、ESP32実機、必要であればArduino Unoなどでも、依存を増やさず同じ考え方でテストできます。

ローカルで主に確認するもの:

- HID report parser
- HID descriptor/report helper
- キーコード変換
- Keyboard LED output report生成
- USB MIDI packet encode/decode
- CDC buffer/Stream互換ロジック
- PCAP TEXT formatter
- descriptor parser
- 状態管理とエラー処理
...
- device接続/切断
- HUB接続とport状態変化
- PPPSによるport power on/off
- HID/CDC/MIDI/MSCなど実デバイスとの通信
- 切断/再接続時のリソース解放
- ログ出力とPCAP TEXT出力

HUBやPPPSなど追加機材が必要なテストは、実施可能な環境でのみ行います。
可能な環境では、複数デバイスをHubに接続したまま、port power on/offで抜き差し相当のテストを自動化します。

UnityやArduTestは必須にはしません。
必要なテストでだけ利用できる補助的な選択肢とし、基本方針としてはテキストベースの `OK` / `NG` 判定を優先します。
