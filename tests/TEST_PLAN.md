...
| ALIENTEK DP100 power supply (HID framed protocol) | | ✅ manual (`dp100`: framed request/response over the HID API, DEVICE_INFO / BASIC_INFO offsets and units, repeated and interleaved reads), ✅ host unit (`unit/dp100`: CRC-16/MODBUS, frame encode/decode, captured-report regression), 🔍 probe (`probe/dp100` established the frame and CRC) | ⬜ the setpoint frame (BASIC_SET), which carries the output enable; SYSTEM_INFO field meanings; SCAN_OUT / SERIAL_OUT |
