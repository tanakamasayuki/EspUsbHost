...
| USB Mass Storage — ブロックI/O / FatFsマウント | ✅ peer（容量、Inquiry/Sense、read/write、範囲外拒否、write失敗検出） | ✅ manual（実USBメモリの容量取得、LBA0 read、FatFs/VFS mount、ファイルwrite/read/delete、mount中disconnect/remount） | ⬜ data phase失敗後の完全なBOT復旧、複数LUN、Arduino `FS`ラッパー、32-bit sector超のFatFs mount |
| 複数デバイス同時接続 | | ✅ manual | |
| デバイス活線挿抜 | | ✅ manual | |
| HUB検出 | | | ⬜（未実装） |
