...
**このディレクトリのファイルを `test_*.py` という名前にしてはいけません。** この命名だけが既定の実行からこれらを外しており、現状の説明ではなく守るべき規約です。手動テストは常時接続されていないハードウェアか、人が見ていることを必要とするので、`pytest` や `pytest manual/` で誤って発火してはなりません。ファイル名はディレクトリ名に合わせます。中の関数には `test_` プレフィックスが必要で、コマンドラインでパスを名指しすればファイル名に関わらず収集されます。
...
| [`vendor_bulk_throughput/`](vendor_bulk_throughput/) | bulk OUTの実効スループット。同期 `vendorWrite()` と非同期キュー（depth 1/2/4/8 × 転送サイズ512 B〜16 KB）を比較し、キューのスロット計上と再利用も検証する。実機のbulk OUT実効上限を確定させる（full-speed / ESP32-S3で1.151 MB/s、high speed / ESP32-P4で38.2 MB/s。10進 MB/s） | vendor-specific (0xff) のbulk OUT endpointを持つ任意のデバイス | ✅ |
| [`uvc_hs_stream/`](uvc_hs_stream/) | ESP32-P4 のハイスピードポートでの UVC isochronous ストリーミング。フルスピードのホストを縛る約120バイトの isochronous IN 上限が、こちらにも当てはまるかを確定させる。カメラは1023バイトの payload を要求するので、**ストリームが流れること自体が答え**になる。peer は 8 KB / 32 KB / 128 KB のフレーム長を自分の周期で巡回し、host は届いたものをフレーム長で仕分けるので、どちらも相手のタイミングを知る必要がなく、device 側のシリアルポート（native USB CDC で、開くとボードがリセットされる）を開かずに済む | `peer_device/` を焼いた2枚目の ESP32-P4。OTG HS 同士を直結 | ✅ payload 1023バイト、128 KBフレームで 7.06 MB/s（2026-09-17） |
| [`vendor_bulk_in_throughput/`](vendor_bulk_in_throughput/) | bulk INの実効スループットと、その律速がどちらにあるか。1パケット/転送のcontinuous readから始め、非同期read queue（depth 1/2/4 × 転送サイズ512 B〜32 KB）を掃引する。全条件でpeerの0..255 rampをバイト単位で検証する。MB/sと並べて `per_transfer` と `starved` を出すので、「deviceがそれ以上出せない」のか「hostが十分訊けていない」のかを切り分けられる | `tests/peer/usb_vendor_read/peer_device` を書き込んだ2枚目のボード | ✅ P4 host + P4 peer、high speed（2026-09-15） |
