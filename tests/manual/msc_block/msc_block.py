...
def test_msc_block_read(dut):
    ...
    dut.expect("MSC_ROOT_ENTRY_COUNT")
    dut.expect(r"MSC_FILE_READ (ok=1|skipped=)")
    dut.expect("MSC_FILE_WRITE ok=1 path=/usb/ESPUSBHT.TST")
    dut.expect("MSC_FILE_READBACK ok=1 path=/usb/ESPUSBHT.TST")
    dut.expect("MSC_FILE_REMOVE ok=1 path=/usb/ESPUSBHT.TST")
    dut.expect("MSC_MOUNT_DUPLICATE ok=0 path=/usb")
    dut.expect("MSC_UNMOUNT ok=1 path=/usb")
    dut.expect("MSC_REMOUNT ok=1 path=/usb")
    dut.expect("MSC_FINAL_UNMOUNT ok=1 path=/usb")
    dut.expect("MSC_FS_BEGIN ok=1 mounted=1 base=/usbfs")
    dut.expect("MSC_FS_WRITE ok=1 path=/ESPUSBFS.TST")
    dut.expect("MSC_FS_READ ok=1 path=/ESPUSBFS.TST")
    dut.expect("MSC_FS_REMOVE ok=1 path=/ESPUSBFS.TST")
    dut.expect("MSC_FS_END mounted=0")
