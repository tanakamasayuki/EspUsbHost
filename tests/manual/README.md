...
| [`vendor_bulk_throughput/`](vendor_bulk_throughput/) | Effective bulk OUT throughput: synchronous `vendorWrite()` against the async queue at depths 1/2/4/8 and transfer sizes 512 B–16 KB, plus queue slot accounting and reuse. Establishes the practical bulk OUT ceiling of the board: 1.151 MB/s at full speed (ESP32-S3), 38.2 MB/s at high speed (ESP32-P4), in decimal MB/s | Any device with a vendor-specific (0xff) bulk OUT endpoint | ✅ |
| [`uvc_hs_stream/`](uvc_hs_stream/) | UVC isochronous streaming on the ESP32-P4 high-speed port. Settles whether the ~120-byte isochronous IN limit that caps a full-speed host applies here too: the camera asks for 1023-byte payloads, so the stream running at all is the answer. The peer steps through 8 KB / 32 KB / 128 KB frames on its own schedule and the host buckets what arrives by frame length, so neither side needs the other's timing and nothing opens the device board's serial port (a native USB CDC, which resets the board when opened) | A second ESP32-P4 flashed with `peer_device/` from this directory, wired OTG HS to OTG HS | ✅ 1023-byte payloads, 7.06 MB/s at 128 KB frames (2026-09-17) |
| [`vendor_bulk_in_throughput/`](vendor_bulk_in_throughput/) | Effective bulk IN throughput and what limits it: the continuous read at one packet per transfer, then the async read queue at depths 1/2/4 and transfer sizes 512 B–32 KB, with the peer's 0..255 ramp verified byte for byte in every condition. Reports `per_transfer` and `starved` alongside MB/s, which separates a device that cannot supply more from a host that is not asking often enough | A second board flashed with `tests/peer/usb_vendor_read/peer_device` | ✅ P4 host + P4 peer, high speed (2026-09-15) |
