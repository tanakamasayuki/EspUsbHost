...
def usb_host(dut, peers):
    ...
    dut.write("G")
    # Deliberately no wait here: this module's tests read output the host prints
    # while it enumerates, and an expect() in this fixture would advance the
    # reader past it. Each test now gets a fresh begin(), so that output is
    # printed per test instead of only once at boot.
    yield
    dut.write("H")
