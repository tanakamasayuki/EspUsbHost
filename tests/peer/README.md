...
- `usb_vendor_read`: the same pairing, for the shape of the bulk IN transfers underneath. The peer streams a 0..255 ramp of a requested length instead of echoing, so the host can check that the same bytes arrive in the same order whether they come one packet at a time, in one 8 KB transfer, or over a read queue with two transfers in flight. Covers the default read size, `vendorOpen()`'s read transfer size and its refusal to be re-opened with a different one, `vendorReadQueueBegin()` / `vendorReadQueueEnd()`, the rejection of invalid queue shapes, and `vendorReadSync()` being refused while the queue owns the endpoint. It is a correctness test: the throughput sweep is `manual/vendor_bulk_in_throughput`, because a full-speed S3 pairing cannot show what the transfer shape is worth on a high-speed bus.
- `usb_video`: pairs with an `EspUsbDeviceVideo` camera from the sibling `EspUsbDevice` library, advertising one MJPEG format at 320x240, 15 fps. Covers UVC end to end: the VideoControl header, the VideoStreaming input header, the Format/Frame descriptor tree flattened into one stream per format/frame pair, `espUsbHostSelectVideoStream()` resolving a caller's request and refusing a size, rate or format the camera does not offer, then Probe/Commit, alternate selection, frame assembly and a clean stop and restart. Also that a camera unplugged mid-stream is enumerated again when it comes back, which is a claim the disconnect path has to release even though the device is already gone. The peer's frames carry `buffer[i] = i + n`, so the host checks the reassembled bytes rather than only their count — a payload header left in the image data, a dropped payload or two frames joined together each break the pattern and none would show up in a byte count. Every descriptor value is compared against a number the peer prints from its own descriptor builder rather than against a constant written into the test, so a decoding mistake shows up as the two boards disagreeing. The peer is deliberately small in two ways, both forced by the core rather than by the library: its configuration descriptor must stay under `CONFIG_USB_HOST_CONTROL_TRANSFER_MAX_SIZE` (256 bytes on arduino-esp32 3.3.11) or the host cannot enumerate it at all, and its `build_opt.h` pins `CFG_TUD_VIDEO_STREAMING_EP_BUFSIZE` to 112 because a full-speed host cannot receive isochronous IN packets much above 120 bytes. The first check asserts the descriptor limit, so the day it stops fitting is reported as a size failure rather than as a decoding failure; because the payload size is a compiler flag, run this test with `--clean` after changing it.
