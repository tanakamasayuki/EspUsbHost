...
def test_usb_serial_write_queue(dut, peers):
...
def test_usb_serial_write_queue_cycles(dut, peers):
    ...
    dut.write("r")
    cycles = dut.expect(
        r"QUEUE_CYCLES completed=([0-9]+) before=([0-9]+) after=([0-9]+) delta=(-?[0-9]+) ready=([0-9]+)",
        timeout=30)
