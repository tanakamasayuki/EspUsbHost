...
static bool handleLifecycle(char command)
{
  if (command == 'h')
  {
    // Free heap, for the allocation cycling test: the per-device vendor state is
    // allocated by vendorOpen() and freed when the device slot resets, so an
    // unbalanced path shows up here and nowhere else.
