...
|---|---|---|---|---|---|---|
| A1 | 既定（約19KB） | 216 | 2.14 | 9% | 57 KB/s | 5% |
| A2 | 32KB | 135 | 2.39 | 10% | 60 KB/s | 5% |
| A3 | 64KB | 64 | **2.53** | 14% | 69 KB/s | 6% |
| A4/A5 | 96KB / 128KB | - | - | - | - | **確保失敗** |
| B1 | 32KB シングル | 135 | 2.41 | 10% | 60 KB/s | 5% |
| B2 | 32KB ダブル | 135 | 2.40 | 10% | 60 KB/s | 5% |
| C1 | diff on | 64 | 2.52 | 13% | 69 KB/s | 6% |
| C2 | diff off | 64 | 1.66 | 100% | 187 KB/s | 17% |
| D1 | clear on | 64 | 2.53 | 13% | 69 KB/s | 6% |
| D2 | clear off | 64 | 2.56 | 14% | 70 KB/s | 6% |
| E1 | 全画面 | 64 | 2.52 | 14% | 69 KB/s | 6% |
| E2 | 240x120 sprite のみ | 1 | **91.56** | - | 165 KB/s | 15% |
| F1 | 直接・全クリア＋全再描画 | - | 5.75 | - | 679 KB/s | 60% |
| F2 | 直接・動いた部分のみ | - | **692** | - | 1123 KB/s | **100%** |
| G1 | 単色 | 64 | 3.65 | 0% | 28 KB/s | 3% |
| G2 | グラデーション | 64 | 3.22 | 0% | 27 KB/s | 2% |
| G3 | UI | 64 | 2.52 | 14% | 70 KB/s | 6% |
| G4 | 疑似ランダム | 64 | 0.27 | 100% | 1123 KB/s | **100%** |

得られた知見:

1. **律速はシーン内容で入れ替わる。** RLE が効く絵（単色・グラデ・UI）はバス使用率 2〜6% で描画律速。ノイズだけがバスを 100% 使って転送律速になる。「Full HD だから転送が重い」ではなく「圧縮できない絵だけが重い」
2. **描画範囲を減らすのが圧倒的に効く。** 全画面 2.52 fps に対し `LGFXVirtualSprite` で変化部分のみ 91.56 fps（36 倍）。他のどのつまみより桁が違う
3. **バンドを減らすと速くなる（+18%）。** 216 → 64 バンドで 2.14 → 2.53 fps。描画コールバックがバンドごとに再実行されるため。ただし Full HD ではシングルタイルバッファの上限が内蔵 RAM で 64KB 程度、96KB 以上は確保失敗
...
- ノイズシーンが描画コスト計測になっていた。1 バンドあたり 64,800 回の `fillRect` で 0.27 fps になり、エンコーダ感度が見えなかった。事前計算した 1 行を `pushImage` する形に変更（結果として同じ 0.27 fps だが、今回は**転送律速**であり測りたいものが測れている）
...
|---|---|---|---|---|---|
| A1 | 既定 | 2.14 | 4.83 | 88 KB/s | 0.2% |
| A5 | 128KB | 確保失敗 | 5.78 | 127 KB/s | 0.3% |
| C2 | diff off | 1.66 | 3.30 | 373 KB/s | 1.0% |
| E2 | sprite のみ | 91.56 | 186.02 | 336 KB/s | 0.9% |
| F1 | 直接・全再描画 | 5.75 | 10.52 | 1242 KB/s | 3.3% |
| F2 | 直接・動いた部分のみ | 692 | 934.54 | 1516 KB/s | 4.1% |
| G4 | 疑似ランダム | 0.27 | 6.15 | 1331 KB/s | 3.6% |

知見:

1. **HS では転送律速の条件が消える。** FS でバスを 100% 使っていた F2 と G4 が、HS ではそれぞれ 4.1% / 3.6%。全条件が CPU 律速になり、ノイズシーンが 0.27 → 6.15 fps（23 倍）と跳ねるのがその現れ
2. **つまみの効き方の順序は変わらない。** どれも描画コストに作用するものなので、バンド削減 1.20 倍、差分転送 1.7 倍、ダブルバッファ 0%、sprite 32 倍と S3 と同じ傾向。P4 でもタイル形状より描画範囲の方が桁違いに効く
3. **P4 では 128KB タイルが確保できる。** S3 では 96KB 以上で確保失敗だったが、P4 は A4/A5 も通り 32 バンドまで減らせる
4. 全体の fps は S3 の約 2 倍。USB ではなく CPU（400MHz デュアル RISC-V）の差
