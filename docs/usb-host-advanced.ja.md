...
`end()` は root port の給電を落とし（ESP32-P4のみ）、このインスタンスがマウントしたMSCボリュームをアンマウントし、クライアントとデーモンを同期的に止め、実行中の転送をキャンセルして排出し、クライアントを登録解除し、IDFの `ALL_FREE` ハンドシェイクを待ってからホストライブラリをアンインストールします。

**root port を最初に、しかもまだ全部が動いているうちに落とす**のには理由があります（自前で teardown を書くなら知っておく価値があります）。device を繋いだままだと controller は port 割り込みを上げ続け、**host library のオブジェクトが解放された後に届いた1つ**が、その割り込み経路の中で落ちます。先に給電を落とせば device は通常の切断になり、まだ生きている task がそれを処理できるので、以降の teardown は空のバスに対して走ります。逆に遅らせて——client を deregister して device を解放し始めた後に落とすと——今度は hub driver が「もう見つからない device の port イベント」を処理することになり、別のクラッシュに化けます。なお実施するのは P4 だけです。同じ呼び出しを ESP32-S3 で行うと**次の** `begin()` が壊れ、しかも S3 の teardown はこの不具合を一度も示していません。アンインストールはIDFが受け付けるまでライブラリイベントを処理しながら再試行します。`NO_CLIENTS` などのイベントフラグが未読の間はIDFが拒否し、device listが空の場合は他に消費するものがないためです。戻った後は同じオブジェクトで `begin()` を再開できます。**USBコールバックの中から呼んではいけません**（アプリケーションタスクから呼ぶ）。
...
### 5.3 フルスピードポートの isochronous IN

**フルスピード**ポートのラインは合計256本しかなく、arduino-esp32 のホストスタックは `CONFIG_USB_HOST_HW_BUFFER_BIAS_PERIODIC_OUT` でビルドされているため、その大半が2つのOUT領域に割かれます。INに残るのは約120バイトです。

この上限はほとんどの場面で見えません。フルスピードのバスでは、**isochronous 以外のエンドポイントはUSB仕様そのものによって64バイト以下**だからです。64バイトを超えて要求できるのは isochronous だけ（最大1023バイト）で、したがってこの上限に当たるのも isochronous だけです。USB Audio のマイクが動くのはこのためで（48 kHz モノラル16ビットのキャプチャエンドポイントは98バイト）、カメラが動かないのもこのためです。

ESP32-S3 のホストと `EspUsbDevice` の UVC カメラで、カメラ側の isochronous パケットサイズだけを変えて実測した結果:

| パケットサイズ | 結果 |
|---|---|
| 96 / 112 / 120 バイト | ストリーム成立。全パケットにデータが入る |
| 128 バイト | `usb_host_interface_claim()` は成功するが、**その後すべてのパケットが `USB_TRANSFER_STATUS_ERROR`** になり、画像データは一切届かない |
| 256 / 512 バイト | `usb_host_interface_claim()` が `ESP_ERR_NOT_SUPPORTED` で失敗 |

128バイトの行が罠で、これは原則として書いておく価値があります。**`usb_host_interface_claim()` が成功しても、そのエンドポイントが使えることの保証にはなりません。** claim が見ているのは転送種別ごとの粗い上限で、実際に受け取れるかを決めるのはFIFOです。その差は「何も出てこないストリーム」としてしか現れません。`EspUsbHostVideoStats::packetErrors` がこれを数え、1転送のパケットが全滅した場合はライブラリがストリームごとに1回警告を出すので、ログに「なぜ何も来ないのか」が残ります。

**device 側にも同じ形の制限があります。**host を疑う前に知っておく価値があります。isochronous IN エンドポイントは device 側でも先に確保される一方、**収まらなくても確保が拒否されない**ため、送信FIFO を超えたカメラは列挙され、ホスト側ドライバにバインドされ、フレーム送出も成功と報告しながら、何も送りません。兄弟ライブラリ `EspUsbDevice` はこの計算を自前で行い、そうした構成を `begin()` の時点で `ESP_ERR_NO_MEM` として拒否します（`transmitFifoFits()`）:

```
使える量 = fifoDepth - 2 * endpointCount        ESP32-S2/S3: 256 - 14 = 242 word
EP0 IN   = ceil(64 / 4)                                     =  16 word
諸経費   = 14 + 2 * (largestOutPacket / 4 + 1) + 2 * endpointCount
                                                            =  62 word （OUT が64バイトのとき）
残り                                                         = 164 word = 656 バイト
```

つまり**両端とも「正しく設定できている」ように見えたまま、ケーブル上を映像が1バイトも流れないことがあります。**見分けるのはどちら側が何か言うかで、**host は `packetErrors` を数え、device は起動を拒否します。**この非対称は覚えておく価値があります。片方は数えないと気づけず、もう片方は起動しないので必ず気づけます。

**この2つは別のFIFOであり、数値を混ぜてはいけません。**前述の2552バイトは *host* コントローラの受信FIFOで、接続中の全デバイスの全INエンドポイントが共有します。ここでの656バイトは *device* コントローラの送信FIFO（S2/S3）で、そのデバイス自身のエンドポイントが共有します。同じ式を ESP32-P4 のハイスピードデバイスに当てると約2688バイトになりますが、これは512バイトのOUTエンドポイントがある前提での値です（`largestOutPacket` が諸経費の項に入るため）。

**arduino-esp32 3.3.11 ではこの分割を変更できません。** `usb_host_config_t::fifo_settings_custom` は ESP-IDF 5.5 で入ったもので、このコアは 5.4 ビルドのため `EspUsbHostConfig::fifo` は無視され、プリコンパイル済みライブラリに焼かれた Kconfig の bias が唯一の分割になります。ESP-IDF 5.5 以降でビルドされたコアなら `rxFifoLines` を上げられます。

**ハイスピード**ポートはこの問題に当たりません。これは計算だけでなく実測で確認しました。ラインは合計1024本あり、既定でも IN に2552バイト残るので、UVC のどの alternate setting が要求する量よりも大きくなります。ESP32-P4 を2枚 OTG HS 同士で直結した構成（`manual/uvc_hs_stream`）では、カメラは**1023バイトの payload** — フルスピード上限の8倍 — で commit し、実際に流れました。
...
全フレームが期待どおりの長さと内容で、失敗した isochronous パケットは80,861 payload 中1つだけでした。ストリーム開始時のもので、カメラのエンドポイントが `SET_INTERFACE` の応答から1マイクロフレームほど遅れて有効になるためです。7.06 MB/s は、1023バイトの isochronous エンドポイント1本が125 µsごとに1パケットを運ぶときの上限 8.18 MB/s の86%です。

**フレームが大きいほど速い**という点は注目に値します。フレームあたりの諸経費は小さく、律速はパケットレートの側だということです。

### 5.4 high-bandwidth isochronous はドライバが未対応

8.19 MB/s は**標準の** isochronous エンドポイントの上限です。125 µs のマイクロフレームあたり1トランザクション、最大1024バイトという意味です。USB 2.0 では周期エンドポイントが1インターバルに2回または3回のトランザクションを要求でき、最大 24.6 MB/s になります。`wMaxPacketSize` の bit 12:11 にその回数から1を引いた値を入れる方式です。**実機の webcam はこれを使い**、ホストが必要最小限だけ予約できるよう alternate を段階的に用意しています。

ESP-IDF 5.4 はこれを動かせません。根拠は3層あり、いずれも設定で変わるものではありません。

| 層 | 状況 |
|---|---|
| `HCCHAR.ec`（Multi Count、bit 21:20） | レジスタ定義に存在する |
| `usb_dwc_hal_ep_char_t` | `mps: 11` のみ。**回数を持つ欄が無い** |
| LL の setter | `mps` / `epnum` / `epdir` / `eptype` / `devaddr` はあるが、**`ec` を書くものが無い** |

持ち運ぶ欄が無いので、チャネルはリセット値の「1インターバル1トランザクション」のままです。

**これを取り違えると静かに壊れます。**だからライブラリ側で無視せずモデル化しています。3トランザクションの alternate にいるカメラは1インターバルに3072バイト送り、ホストは1024バイトしか取りません。payload が途中で切られ、フレームは短く届き、**どこにもエラーが出ません** — stall も、失敗パケットも、overflow もです。そこで alternate は2つの値で記録します。`payloadSize` はディスクリプタが提示する値、`usablePayload` はこのコントローラが実際に取れる値です。選択と転送サイズ決定は後者を使うので、multiplier を要する alternate は素のパケットサイズで順位付けされ、**全部使える小さい alternate があればそちらが勝ちます。**交渉した payload を賄えるのが high-bandwidth の alternate しか無い場合は、短いフレームを黙って流さずログに出します。

切り替えは `ESP_USB_HOST_ISOC_HIGH_BANDWIDTH_SUPPORTED` の1箇所です。既定は0で、コアが対応したらスケッチの `build_opt.h` で1にすれば、`usablePayload` は全経路で `payloadSize` と等しくなります。**新しいコアでの再確認方法**は、`usb_dwc_hal_ep_char_t` に回数の欄があるか、および LL に HCCHAR の `ec` を書く setter があるかの2点です。両方揃っている必要があります。

それまでの実用上の読み方は、**このホストは1マイクロフレームあたり1024バイトまでを要求するカメラに対応できる**、ということです。3〜6 MB/s の 1080p30 MJPEG には足りますが、段を持つ webcam の最上段には足りません。

**フレームコールバックの中で時間を使うと、その分がそのままこの数値から引かれます。**コールバックは USB client task で動き、それは転送を再投入するタスクでもあります。転送4本×8パケットは、ハイスピードではわずか4 ms分のキューにすぎません。この測定の初期版はコールバック内で128 KB全体を検証していました。そのときの128 KB行は **3.47 MB/s** — 32 KB行の半分 — で、フレームは約6 KB短く届いていました。コールバックが走っている間に到着したマイクロフレームに行き先が無かったためです。**エラーはどこにも出ません。**フレームの中身が減るだけです。フレームはコピーして、すぐ返してください。
