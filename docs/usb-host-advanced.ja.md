...
**ハイスピード**ポートはこの問題に当たらないはずです。ラインは合計1024本あり、既定でも IN に2552バイト残るので、UVC のどの alternate setting が要求する量よりも大きくなります。ただしこれは上記のFIFO計算からの帰結であって実測ではありません。この節の数値はフルスピードの ESP32-S3 ペアで取ったもので、**UVC を ESP32-P4 のハイスピードポートで動かした実測はまだありません**。
