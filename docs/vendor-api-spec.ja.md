...
> API added `vendorOpen()` read modes and a configurable read transfer size,
> `vendorControlTransfer()` with a caller-supplied `bmRequestType`, asynchronous write and
> read queues, and a configurable receive buffer. For the
