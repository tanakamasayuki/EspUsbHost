...
電源なので**書き込み系 OpCode は撃たない**。`BASIC_SET (0x35)` は出力 ON/OFF を含むため probe の対象外とした。
...
- 副産物として、**拒否されたフレームは無視されず、OpCode エコー + 1 バイトの `0x00` で応答される**ことが判明した。無応答と区別できるので、フレームの組み立て間違いを検出できる。`isRefusal()` として実装した
...
- 設定値の書き込みと読み戻し — **未達成（意図的に未検証）**

## 残っている未確認事項・リスク

1. **`BASIC_SET (0x35)`**。設定値と出力 ON/OFF を 1 フレームに載せているため、要求の形を当てずっぽうで探索していない。検証には「出力端子に何も接続されていない」ことの確認が必要
2. **iout のスケール**。出力 OFF で 0 しか観測していないので mA と断定できていない
