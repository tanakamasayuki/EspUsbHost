...
実務上重要なのは最後の行で、チューニングの指針が逆になります。DL-1xxアダプタではバスは数%しか使われず描画がコストなので、バンドは少なく大きくするのが有利です。スマートスクリーンでは1ピクセルがワイヤ上で2バイトかかり、パネルが0.163 MB/sの固定レートでペースを決めるため、効くのは送るピクセルを減らすことだけです。矩形への分割の仕方はコストを変えません。AX206ではそれすら効きません。送るピクセル数を選ぶ余地がないからです。各exampleのREADMEに実測のスイープがあります。
