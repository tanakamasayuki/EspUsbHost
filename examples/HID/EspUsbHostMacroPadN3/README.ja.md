...
```
"CRT" 00 00 | コマンド3文字 | 引数 | 1024バイトまでゼロ埋め
```
...
ノブの番号はこのexample独自のもので、押し込みコード順です。S6ではノブ1が左下、2が右下、3が上中央になります。他ブランドの個体では同じコードが別の位置に割り当てられている可能性があるので、位置が重要なスケッチは自分の個体で確認してください。
...
実際に踏んで分かったこと: キーを押すとパッドはアップロードしたタイルの上に内蔵アイコンを描き直します。スケッチが入力で再描画するのはこのためです。

同じ個体を純正アプリが駆動しているUSBキャプチャ（`11.pcapng`、USBフレーム55k）から読み取った内容: 起動シーケンス `DIS` / `LIG 25` / `QUCMD` / `CLE ff` / `BAT`×6 / `STP`、`CONNECT` keepaliveの間隔9.4〜10.0秒、入力レポートのヘッダとキー・ノブのコード、そしてアプリが画像保持に頼らず継続的に再アップロード＋リフレッシュしていること。
