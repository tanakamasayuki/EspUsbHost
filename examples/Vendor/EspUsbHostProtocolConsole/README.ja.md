...
想定する手順は次のとおりです。PC上でUSBPcap + Wireshark（LinuxならUSBmon）でキャプチャし、その中の1つの転送を選び、ここでバイト単位に再現します。応答がキャプチャと一致すれば、そのプロトコルの一部は理解できたことになり、スケッチに書き起こせます。[docs/usb-host-guide.ja.md](../../../docs/usb-host-guide.ja.md) の手順の一部です。
