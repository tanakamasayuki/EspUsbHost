...
これは**デバイスが2台**で、1台あたり1ポートの構成です。1本のケーブルで複数のCDCポートを見せる複合デバイスの場合は、`setAddress()`ではなく`setPort()`でストリームをバインドします。メインREADMEの[1デバイスに複数のCDCポート](../../../README.ja.md#1デバイスに複数のcdcポート)を参照してください。
